...
/**
 * The Class MutationDictonary. the class is using a java
 * {@link Hashtable} only allowing to add multiple items in one key.
 * (By chaining method using {@link LinkedList})
 * 
 * 
 * @see Hashtable
 * @see LinkedList
...
public class MutationLibrary {
	...
	/**
	 * Instantiates a new mutation dictionary.
	 */
	...
	/**
	 * put the element in the table in the end of the list for the key but does
	 * not allow the same object in the same list
	 * 
	 * @param key
	 *            the key
	 * @param element
	...
	public ArrayList<MutationLibraryEntry> get(Vector3f key) {
		ArrayList<MutationLibraryEntry> out = new ArrayList<MutationLibraryEntry>(
				data.get(key));
		return out;
	}
